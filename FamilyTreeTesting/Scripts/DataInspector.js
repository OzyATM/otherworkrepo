...
Inspector.prototype.buildPropertyRow = function (propertyName, propertyValue) {
    ...
    if (propertyName === "bold")
        ...
        td1.textContent = "斜體";
    else if (propertyName === "isUnderline")
        td1.textContent = "底線";
    else if (propertyName === "isStrikethrough")
        td1.textContent = "刪除線";
    else if (propertyName === "fontsize")
        td1.textContent = "字型大小";
    else if (propertyName === "stroke")
        td1.textContent = "字體顏色";
    else if (propertyName === "text")
        td1.textContent = "輸入文字";
    else
    ...
    if (decProp !== undefined && decProp.type === "checkbox") {
        input.setAttribute("type", "button");
        if (input.type === "button") {
            input.addEventListener("input", setprops);
            input.addEventListener("change", setprops);
            input.value = this.setValue(propertyValue);
        }
    }
    if (this._diagram.model.isReadOnly) input.disabled = true;

    if (input.type !== "color" && input.type !== "button") input.addEventListener("blur", setprops);
...
Inspector.prototype.setValue = function (propertyValue) {
    var chk = document.createElement("input");
    chk.checked = propertyValue;
    return chk.checked;
}
    ...
    if (!data) {  // clear out all of the fields
        ...
        for (var name in inspectedProps) {
            ...
            if (input.type === "color") {
                ...
                input.checked = this.setValue(propertyValue);
            }
            else {
                input.value = propertyValue;
            }
...
Inspector.prototype.updateAllProperties = function () {
    ...
    for (var name in inspectedProps) {
        if (name == "isUnderline" || name == "bold" || name == "Italic" || name == "isStrikethrough") {
            var value = inspectedProps[name].checked;
        }
        else {
    ...
    self.getRadio(this.inspectedObject);
};
