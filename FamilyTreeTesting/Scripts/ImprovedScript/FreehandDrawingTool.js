...
function FreehandDrawingTool() {
  ...
  this._temporaryShape = go.GraphObject.make(go.Shape, { name: "SHAPE", fill: null, strokeWidth: 2 }); 
...
FreehandDrawingTool.prototype.doMouseUp = function() {
  ...
  var newGeoData = "";
  for (i = firstL; i < SecondL - 1; i++)
  {
      newGeoData = newGeoData + GeoData[i];
  }
  GeoData = GeoData + " " + newGeoData;
  d.geo = GeoData;
  this.stopTool();
